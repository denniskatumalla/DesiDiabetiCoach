...
- **Supabase types are hand-derived, not generated.** Every client is now typed `SupabaseClient<Database>` (web server/browser/admin, tRPC context, mobile), so query results are compile-time checked against the schema. The `Database` type in `packages/shared/src/database.types.ts` was written by hand from `supabase/migrations/*.sql` because generating it needs a running database, and Docker is unavailable on the current dev machine (VMware guest, nested virtualisation disabled). It deliberately matches the CLI's output shape, so once a real project exists it can be replaced wholesale:
  `supabase gen types typescript --linked > packages/shared/src/database.types.ts`
  Until then, **a new migration means updating that file by hand.** Note that columns constrained by `CHECK` (rather than a Postgres `ENUM`) are typed `string`, exactly as the generator emits them — the narrow unions live in this package's Zod schemas and are re-applied at the API boundary.
